...
public class CodeGenerator
{
    ...
    public string GenerateClass(GdClass gdClass)
    {
        ...
        // Collect all StringName constants needed
        var stringNames = new Dictionary<string, string>(); // snake_case -> ConstantName
        foreach (var prop in gdClass.Properties)
        {
            stringNames[prop.Name] = $"_property{TypeMapper.ToPascalCase(prop.Name)}";
        }
        foreach (var method in gdClass.Methods)
        {
            stringNames[method.Name] = $"_method{TypeMapper.ToPascalCase(method.Name)}";
        }
        foreach (var signal in gdClass.Signals)
        {
            stringNames[signal.Name] = $"_signal{TypeMapper.ToPascalCase(signal.Name)}";
        }
        ...
        // Generate StringName constants
        if (stringNames.Count > 0)
        {
            sb.AppendLine("    #region StringName Constants");
            sb.AppendLine();
            foreach (var kvp in stringNames.OrderBy(k => k.Value))
            {
                sb.AppendLine($"    private static readonly StringName {kvp.Value} = \"{kvp.Key}\";");
            }
            sb.AppendLine();
        ...
        foreach (var prop in gdClass.Properties)
        {
            GenerateProperty(sb, prop, stringNames);
        }
        ...
        if (gdClass.Signals.Count > 0)
        {
            GenerateSignals(sb, gdClass.Signals, stringNames);
        }
    ...
    private void GenerateProperty(StringBuilder sb, GdProperty prop, Dictionary<string, string> stringNames)
    {
        var csharpType = TypeMapper.GetCSharpType(prop.Type);
        var propName = TypeMapper.ToPascalCase(prop.Name);
        var stringNameConst = stringNames[prop.Name];
        ...
        var getterCode = TypeMapper.GetFromGodotConversion($"_instance.Get({stringNameConst})", prop.Type);
        ...
        if (prop.HasSetter)
        {
            ...
            sb.AppendLine($"        set => _instance.Set({stringNameConst}, {setterConversion});");
            sb.AppendLine("    }");
        }
        else
        {
    ...
    /// </summary>
    private void GenerateAsyncMethod(StringBuilder sb, GdMethod method, GdSignal signal, List<string> paramList, Dictionary<string, string> stringNames)
    {
        var methodName = TypeMapper.ToPascalCase(method.Name);
        var signalName = TypeMapper.ToPascalCase(signal.Name);
        var paramsStr = string.Join(", ", paramList);
        var methodStringNameConst = stringNames[method.Name];
        ...
        var callArgs = new List<string> { methodStringNameConst };
    ...
    private void GenerateSignals(StringBuilder sb, List<GdSignal> signals, Dictionary<string, string> stringNames)
    {
        ...
        {
            var eventName = TypeMapper.ToPascalCase(signal.Name);
            var signalStringNameConst = stringNames[signal.Name];

            if (signal.Parameters.Count == 0)
            {
                sb.AppendLine($"        _instance.Connect({signalStringNameConst},");
